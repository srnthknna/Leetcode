    ...
    private int countLessEqual(int[] smallLargePair, int[][] matrix, int mid) {
        ...
        while (row >= 0 && col < n) {
            if (matrix[row][col] > mid) {
                // store the first value min and greater than mid
                smallLargePair[1] = Math.min(smallLargePair[1], matrix[row][col]);
                row--;
            } else {
                // find the first value greater than or equal to mid
                smallLargePair[0] = Math.max(smallLargePair[0], matrix[row][col]);
                // its a square matrix so we only add the triangular less values intelligently
                count += row + 1;
                col++;
            }
        }
