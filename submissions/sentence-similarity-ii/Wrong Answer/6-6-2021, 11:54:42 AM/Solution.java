...
class Solution {
    public boolean areSentencesSimilarTwo(String[] sentence1, String[] sentence2, List<List<String>> similarPairs) {
        ...
        for (int i = 0; i < sentence1.length; i++) {
            ...
            else if (!words.containsKey(sentence1[i])) return false;
            else if (!words.containsKey(sentence2[i])) return false;
            else if (!uf.isConnected(words.get(sentence1[i]), words.get(sentence2[i]))) return false; 
        }
        return true;
    }
